...
blogRouter.post('/', async (request, response) => {
  ...
  if (user) {
    ...
    response
      .status(401)
      .json({ error: 'User authorisation is required to create blogs' })
  }
})
...
blogRouter.delete('/:id', async (request, response) => {
  ...
  if (user) {
    ...
    if (user._id.toString() === toBeDeleted.user.toString()) {
      ...
      response
        .status(403)
        .json({ error: 'user is not authorised to delete blog' })
    }
  } else {
    response
      .status(401)
      .json({ error: 'User authorisation is required to delete blogs' })
  }
})
...
blogRouter.put('/:id', async (request, response) => {
  ...
  if (user) {
    ...
    response
      .status(401)
      .json({ error: 'User authorisation is required to update blogs' })
  }
})
