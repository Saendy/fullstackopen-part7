...
test('deleting a blog is successful', async () => {
  ...
  await api
    .delete(`/api/blogs/${blogsResponse.body[0].id}`)
    .set('authorization', `Bearer ${token}`)
    .expect(204)
...
test('updating a blog is successful', async () => {
  ...
  await api
    .put(`/api/blogs/${blogsResponse.body[0].id}`)
    .set('authorization', `Bearer ${token}`)
    .send(newBlog)

  const updatedBlogsResponse = await api.get('/api/blogs')
  expect(updatedBlogsResponse.body[0].likes).toBe(
    blogsResponse.body[0].likes + 1
  )
})
